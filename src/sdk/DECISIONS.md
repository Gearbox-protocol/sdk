...
- ~~**Onchain-first by default**, as an explicit per-field source policy per namespace with
  ...
  otherwise the chain wins. `mergeChainList` / `mergeChainOne` in `src/sdk/utils` are
...
- `DataResponse` describes a backend **2xx body only**. A non-2xx, a timeout, or a body that
  ...
  any chain (§6).
- **The envelope is no longer confined to `src/sdk`.** It is the return type of
  `src/model`'s codec, of the fan-out services in `src/onchain`, and of the backend client in
...
- `src/sdk` — `GearboxSDK` (combined entry point) and its namespaces: routing, the
...
- `src/onchain/<namespace>` — the onchain read service for a namespace, wired onto `OnchainSDK`
...
- **Amended (sdk-first): two more directories under `sdk/`** — `prepare/` and
  `execute/` — glue from the read-model-shaped request to `src/onchain`'s
...
- **Errors extend viem's `BaseError`**, as `src/onchain` already does, and use its layering:
...
- **"Onchain data is a subset of offchain data" is a rule we enforce, not something that is
  ...
  left in the low-level `src/onchain` services. It must not end up in the shared read model as a
  third kind of field.
...
- **Curated classification tables are a second cross-service contract.** `AssetType`,
  ...
  `src/onchain/chain/chains.ts`, with lookup helpers next to `getCuratorName`. That file is the
