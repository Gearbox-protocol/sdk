...
/**
 * The fan-out itself: which block a request is read at, what its metadata
 * reports, and which failures a chain absorbs.
 *
 * The public props of a fan-out method never carry a block — a height is not a
 * shared moment across networks — so the block each chain is read at is resolved
 * here, once per chain, and handed to the single-chain read.
 **/
