...
import { type CreditAccountData, json_parse, OnchainSDK } from "../../index.js";
...
describe("close/repay with withdrawals (WETH strategy)", () => {
  ...
  it("previews closing the account", async () => {
    ...
    await expect(preview(CLOSE)).resolves.toMatchObject({
  ...
  it("previews repaying the account", async () => {
    ...
    await expect(preview(REPAY)).resolves.toMatchObject({
      ...
      collateralWithdrawn: [
        // entire weETH balance returned in-kind
        {
          ...
          value: 45_511_693_717_367_280_721n,
        },
  ...
  spec => {
    ...
    it("previews the full repay as RepayCreditAccount", async () => {
      ...
      expect(preview).toMatchObject({
        ...
        collateralAdded: [
          {
            token: expect.objectContaining({ address: spec.repayToken }),
            value: spec.repayAmount,
          },
        ],
