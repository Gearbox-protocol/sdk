...
transaction calldata into a typed, human-readable operation and check the
conditions the sender must satisfy for it to succeed.
...
The [`simulate`](./simulate) subdir runs the call against real chain state to
recover what actually would happen (balance changes / transfers, or the revert
reason). It relies on `eth_simulateV1`, which is **only available on Mainnet**, so
it cannot be used uniformly across the networks the SDK supports. For that reason
it is **not exported** from this module's public API.

The code is kept internally for now because it may be useful for future tests.
