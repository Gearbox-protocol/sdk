...
/**
 * The decision table of the freshness merge, one case per row.
 *
 * Everything here is a pure function over metadata, so the fixtures are just
 * chain entries: a row belongs to a chain, and its chain's entry is what decides
...
describe("mergeChainList decides each chain on its own", () => {
  ...
  it("honours a threshold the caller chose over the default", () => {
    const behind = (NOW - 60) as Timestamp;
    const onchain = response(
      [succeeded(1, "onchain", NOW)],
      row(1, "onchain row"),
    );
    const offchain = response(
      [succeeded(1, "offchain", behind)],
      row(1, "offchain row"),
    );

    expect(mergeChainList(onchain, offchain, 10)?.data).toEqual([
      row(1, "onchain row"),
    ]);
    expect(mergeChainList(onchain, offchain, 120)?.data).toEqual([
      row(1, "offchain row"),
    ]);
  });
  ...
  it("treats a backend that does not say when its data is from as stale", () => {
    const merged = mergeChainList(
      response([succeeded(1, "onchain", NOW)], row(1, "onchain row")),
      response([succeeded(1, "offchain")], row(1, "offchain row")),
    );

    expect(merged?.data).toEqual([row(1, "onchain row")]);
  });

  it("prefers the backend when the chain does not say either", () => {
    // the usual reason is a chain the SDK never attached to, which has rows from
    // nowhere and a block from nowhere
    const merged = mergeChainList(
      response([succeeded(1, "onchain")], row(1, "onchain row")),
  ...
  it("keeps a chain only one source reported as that source's failure", () => {
    const merged = mergeChainList(
      response([failed(1, "onchain")]),
      response([], row(1, "offchain row")),
    );

    const chain = merged?.meta.chains[0];
    expect(chain?.status).toBe("error");
    expect(chain?.source).toBe("onchain");
    // metadata decides which chains exist: rows of a chain the backend did not
    // report cannot be placed on either side of the comparison
...
describe("mergeChainOne applies the same rule to one entity", () => {
  ...
  it("has nothing to answer with when no source served it", () => {
    // an entity has no partial stand-in, so the caller is what decides whether
    // this is a failure or a read still in flight
    expect(
