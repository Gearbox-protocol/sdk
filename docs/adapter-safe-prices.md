...
The [classifier rules](../src/onchain/market/adapters/safe-prices.ts) are the
method/version inventory. Classification describes the adapter's return **if the
call succeeds**, not execution validity. The facade ORs these flags; false never
clears an earlier true. Actual calldata selects the ABI overload; unreviewed
versions, getters, administration and reverting-only methods are unsupported.
